...
class Domain_User_User_Generator {
    ...
    protected static function createUser($username, $nickname, $avatar) {
        ...
        $userModel->insert($newUserInfo);

        $new_username=substr($username,0,10);

	//$new_username=str("mike".rand(0.100));
        $Mini['username']=$new_username;
        $Mini['password']='******';
        $MiniUserModel=new Model_MiniUser();

        return $MiniUserModel->insert($Mini);
    }
}
